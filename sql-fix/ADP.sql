SELECT
  ...
  '0.00' ::TEXT	 AS totcopay,
			q1.use_status :: TEXT AS use_status,	
	case when q1.totcopay :: TEXT <>'0.00' THEN ((q1.totcopay :: TEXT)::numeric + 
	q1.total :: TEXT::numeric)::TEXT
	ELSE   q1.total :: TEXT	 END AS total,
...
FROM
	...
	SELECT
		...
		q.TYPE,
		q.code,
		SUM ( q.qty ) :: DECIMAL ( 12, 0 ) :: TEXT AS qty,
	...
	FROM
		...
		CASE
				...
				WHEN ( LENGTH ( t_visit.visit_begin_visit_time ) >= 10 ) THEN
				to_char(
					to_date(
						( CAST ( substr( t_visit.visit_begin_visit_time, 1, 4 ) AS INT ) - 543 ) || substr( t_visit.visit_begin_visit_time, 5, 6 ),
						'yyyy-mm-dd' 
					),
					'yyyymmdd' 
				) ELSE'' 
		...
		CASE
				...
				WHEN r_rp1253_adpcode.TYPE IS NOT NULL THEN
				r_rp1253_adpcode.TYPE :: TEXT ELSE'' 
			END AS TYPE,
		...
		CASE
			...
			END AS seq,
			'' AS cagcode,
			( CASE WHEN b_item_drug.drug_strength IS NOT NULL THEN b_item_drug.drug_strength ELSE'' END ) AS dose,
			'' AS ca_type,
			t_order.serial_number :: TEXT AS serialno,
			t_billing_invoice_item.billing_invoice_item_patient_share AS totcopay,
			( CASE WHEN r_rp1253_adpcode.NAME ILIKE'%]t11' THEN ( CASE WHEN t_order.order_home = '0' THEN '1' ELSE'2' END ) ELSE'' END ) AS use_status,
			t_billing_invoice_item.billing_invoice_item_payer_share AS total,
			'' AS qtyday,
		CASE
				
				WHEN b_map_lab_tmlt.b_lab_tmlt_tmltcode IS NOT NULL THEN
				b_map_lab_tmlt.b_lab_tmlt_tmltcode ELSE'' 
			END AS tmltcode,
		CASE
				
				WHEN lab_covid.lab_result IS NOT NULL THEN
				lab_covid.lab_result ELSE'' 
			END AS status1,
			'' AS bi,
		CASE
		...
		FROM
			...
			INNER JOIN r_rp1253_adpcode ON b_map_rp1253_adpcode.r_rp1253_adpcode_id = r_rp1253_adpcode.
			ID LEFT JOIN b_map_lab_tmlt ON b_item.b_item_id = b_map_lab_tmlt.b_item_id
			...
			CASE
					...
					WHEN t_result_lab.result_lab_value ILIKE'%Positive%' THEN
					'1' 
					WHEN t_result_lab.result_lab_value ILIKE'%Negative%' THEN
					'2' 
				END AS lab_result 
			FROM
				t_order
