...
FROM
	...
	FROM
		...
		INNER JOIN t_ipd ON t_ipd.an = t_adp.an
		INNER JOIN t_pat ON t_pat.hn = t_ipd.hn 
	WHERE
		t_adp.an = (SELECT vn FROM vn)
	GROUP BY
		t_adp."id",
		t_adp.hn,
		t_adp.an,
...
t_dru.person_id,
 '' as seq
FROM t_dru 
INNER JOIN t_ipd ON t_ipd.an = t_dru.an

WHERE t_dru.an = (SELECT vn FROM vn)
